...
if (isset($_GET['delete'])) {
  ...
  if ($deletestmt->execute()) {
    ...
    header("refresh:1; url=./ResultStudent.php");
  }
}
...
<head>
  ...
  <title>หน้ารายงานสรุปนักศึกษาที่ลงทะเบียนเรียนในแต่ละภาคการศึกษา</title>
  ...
  <div class="row">
    ...
    <div class="row">
      <main class="col-md-9 ml-sm-auto col-lg-10 px-md-3 py-3">
        ...
        <h1 class="h2" style="font-family: 'IBM Plex Sans Thai', sans-serif;">ข้อมูลรายงานสรุปนักศึกษาที่ลงทะเบียนเรียนในแต่ละภาคการศึกษา</h1>
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb fs-5 mt-3 ms-3">
            ...
            <li class="breadcrumb-item active" aria-current="page">รายงานสรุปนักศึกษาที่ลงทะเบียนเรียนในแต่ละภาคการศึกษา</li>
          </ol>
        </nav>
        ...
        <div class="col-12 col-xl-8 mb-4 mb-lg-0" style="width: 100%;">
          <div class="card shadow-sm">
            ...
            <div class="card-body">
                  ...
                  <tbody>
                    ...
                    if (isset($_POST['submitfilter'])) {
                      ...
                      if (empty($selectedYear) && empty($selectedTerm) && empty($selectedGroup)) {
                        ...
                        $stmt->execute();
                        $users = $stmt->fetchAll();
                        $selected = 'นักศึกษาทั้งหมดในระบบ';
                        ...
                        $stmt->execute();
                        $users = $stmt->fetchAll();
                        $selected = 'นักศึกษาทั้งหมดในภาคการศึกษาที่ ' . $selectedTerm . ' กลุ่ม ' . $selectedGroup;
                      } elseif (!empty($selectedYear) && empty($selectedTerm) && !empty($selectedGroup)) {   //ใส่ selectedTerm และ selectedGroup
                        ...
                        $stmt->execute();
                        $users = $stmt->fetchAll();
                        $selected = 'นักศึกษาทั้งหมดในปีการศึกษาที่ ' . $selectedYear . ' กลุ่ม ' . $selectedGroup;
                        ...
                        $stmt->execute();
                        $users = $stmt->fetchAll();
                        $selected = 'นักศึกษาทั้งหมดในปีการศึกษา ' . $selectedYear . ' ภาคการศึกษาที่ ' . $selectedTerm;
                      } elseif (!empty($selectedYear) && !empty($selectedTerm) && empty($selectedGroup)) {   //ใส่ selectedYear และ selectedTerm
                        ...
                        $stmt->execute();
                        $users = $stmt->fetchAll();
                        $selected = 'นักศึกษาทั้งหมดในปีการศึกษา ' . $selectedYear;
                      } elseif (empty($selectedYear) && !empty($selectedTerm) && empty($selectedGroup)) {   //ใส่ selectedYear และ selectedTerm
                        $sql = "SELECT student.*
                        FROM `student`
                        LEFT JOIN `groups` ON student.group_id = groups.group_id
                        ...
                        $stmt->execute();
                        $users = $stmt->fetchAll();
                        $selected = 'นักศึกษาทั้งหมดในภาคการศึกษาที่ ' . $selectedTerm;
                      } elseif (empty($selectedYear) && empty($selectedTerm) && !empty($selectedGroup)) {   //ใส่ selectedYear และ selectedTerm
                        $sql = "SELECT student.*
                        FROM `student`
                        LEFT JOIN `groups` ON student.group_id = groups.group_id
                        ...
                        $stmt->execute();
                        $users = $stmt->fetchAll();
                        $selected = 'นักศึกษาทั้งหมดกลุ่ม ' . $selectedGroup;
                        ...
                        $stmt->execute();
                        $users = $stmt->fetchAll();
                        $selected = 'นักศึกษาทั้งหมดในปีการศึกษา ' . $selectedYear . ' ภาคการศึกษาที่ ' . $selectedTerm . ' กลุ่ม ' . $selectedGroup;
                      } else {
              ...
              <?php
              if (!empty($users)) { ?>
                <form action="./PDF_Result/PDF_ResultStudent.php" method="post" target="_blank">
